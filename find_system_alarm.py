...
def peredaem_ess_alarm_slovar():
    return ess_timers_slovar


def add_ess_alarm_timers(datatime, tag_system):
    print("------------------------------------")
    print("add__ess__alarm_timers()")

    # проверка на наличие записи ключа(тега) сиситемы
    print("# проверка на наличие записи ключа(Tега) системы = " + tag_system)

    if tag_system in ess_timers_slovar:
        print("ЗАПИСЬ ECТЬ = " + tag_system + " : " + (str(ess_timers_slovar[tag_system])))

        # Обновляем запись всёравно
        ess_timers_slovar[tag_system] = datatime

        # если запись есть надо проверить сколько прошло времени с того момента и отреагировать на это
        ess_sravnivaem_datatime(datatime, tag_system)
...
def ess_sravnivaem_datatime(may_datatime, tag_system):
    print("  ******  def ess_sravnivaem_datatime ********** ")

    save_data = ess_timers_slovar[tag_system]
    print(save_data)
    # save_data_str = save_data.strftime('%Y, %m, %d, %H, %M')
    # print( save_data_str )
    naw_data = datetime.strptime(may_datatime, "%Y, %m, %d, %H, %M");
    ...
    if save_data == naw_data:
        ...
        print("ess прошло " + (str(i)) + " минут, " + str(test_datatime_4.seconds) + " секунд для ESS системы: " + tag_system)
        ...
        if i < 5:
            ...
            del_ess_system_alarm(tag_system)
            return [1, i]


def del_ess_system_alarm(systema_alarm_tag):
    if systema_alarm_tag in ess_timers_slovar:
        del ess_timers_slovar[systema_alarm_tag]
        print(f"Элемент с ключом {systema_alarm_tag} удален")
    else:
        print("Элемент не найден при попытки удалить из ESS словаря систему  " + systema_alarm_tag)
